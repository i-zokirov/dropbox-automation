...
class Task {
    ...
    async getSingleTaskList(tasklist) {
        const { data } = await this.taskapi.tasklists.get({ tasklist });
        return data;
    }
    ...
    async getOpenTasks(tasklistId) {
        const data = await this.getTasks(tasklistId);
        return data.filter((item) => item.status === "needsAction");
    }

    async updateTask(taskId, tasklistId, body) {
        const { data } = await this.taskapi.tasks.patch({
            task: taskId,
            tasklist: tasklistId,
            requestBody: { ...body },
        });
